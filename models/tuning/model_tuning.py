...
def ray_tune_train_model(
    ...
    model = set_parallel(model, str(device))
    ...
    for epoch_steps, epoch in enumerate(range(start_epoch, n_epochs), start=1):
        running_loss = 0.0
        # mini-batch training loop
        for i, (train_X, train_y) in enumerate(train_loader, start=1):
            train_X, train_y = train_X.to(device), train_y.to(device)
            # forward step, loss calculation, and backprop
            curr_loss = model.partial_fit(train_X, train_y)
            running_loss += curr_loss.item()

            if i % 1000 == 0:
                print(
                    f"Epoch: {epoch_steps}, minibatch {i:5d}: Avg. Epoch Loss: {running_loss / i}"
                )
                running_loss = 0.0
        # validation loss
        test_loss = 0.0
